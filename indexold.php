...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['Descargar'])) {
    ...
    if (!filter_var($url, FILTER_VALIDATE_URL)) {
        ...
        // Verifica si puede ser vídeo con yt-dlp --simulate
        exec("yt-dlp --simulate --get-title --get-duration '$url' 2>&1", $salida, $status);
        if ($status !== 0) {
            mostrar_mensaje("La URL no corresponde a un vídeo válido.");
            registrar_log('DESCARGA_URL', $url, 'FALLO_SIM');
        } else {
...
// Progress AJAX
if (isset($_GET['progress']) && isset($_GET['file'])) {
    $file = preg_replace('/[^A-Za-z0-9._-]/', '_', $_GET['file']);
    $out_file = OUT_DIR . preg_replace('/\..*$/', '.mus', $file);
    echo file_exists($out_file) ? '100' : '10';
    exit;
}
?>
...
<head>
  ...
  <script>
    ...
    // Progreso de conversión mediante AJAX polling
    function progreso(file) {
      var barra = document.getElementById('barra');
      var prog = document.getElementById('progreso');
      barra.style.width = '10%';
      prog.style.display = 'block';
      var interval = setInterval(() => {
        fetch('index.php?progress=1&file='+encodeURIComponent(file)).
        then(res => res.text()).
        then(p => {
          barra.style.width = p + '%';
          if(p=='100') {
...
<div class="container">
  ...
  <div style="margin-left:220px;">
    ...
    <form enctype="multipart/form-data" action="index.php" method="POST" onsubmit="progreso(subir_archivo.value)">
      <fieldset>
